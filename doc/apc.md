...
- **`calSF`**
    ...
    输入3（可选）：`double`，指定最小的截断的 q（过小的结果会发散，默认为
    0.6 倍*单位长度*）
...
- **`calSF_AB`**
    ...
    输入3（可选）：`double`，额外指定最大计算的 q 的位置（默认为 6 倍*单位长度*）
    
    输入4（可选）：`double`，指定最小的截断的 q（过小的结果会发散，默认为
    0.6 倍*单位长度*）
...
- **`calConnectRatioBOOP`**
    ...
    描述：通过 BOOP（Ql）来计算结构中每个原子的连接数占所有近邻数的比例值，
...
- **`calConnectRatioABOOP`**
    ...
    描述：通过 ABOOP（ql）来计算结构中每个原子的连接数占所有近邻数的比例值，
    输出结果为按照输入原子顺序排列的向量，数值为 0~1 的比例值。
