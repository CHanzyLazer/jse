...
一般情况可以直接使用程序内部预置的任务提交器，在 groovy 脚本中，可以使用类似语句导入：
```groovy
...
--------------------------------
和 matlab 类似，使用 `system()` 方法来执行系统指令从而提交任务，基本流程为（groovy 代码，下同）：
```groovy
// 全局任务提交器
import static com.jtool.code.CS.Exec.EXE;
...
默认情况下，一般的任务提交器会将指令的输出直接输出到控制台（`java.lang.System.out`），如上述代码则会在控制台中输出：
```
123456
exitValue: 0
```
...
--------------------------------
由于全局的任务提交器 `com.jtool.code.CS.Exec.EXE` 在程序中其他地方也有使用，因此**不能**手动关闭。

--------------------------------
现在这些任务提交器会在程序结束后自动关闭，在一些简单的脚本中可以省略掉 `shutdown()` 语句，但是依旧建议及时手动关闭（`shutdown()` 或者使用 `try-with-resources`），保证程序能够及时释放资源。
