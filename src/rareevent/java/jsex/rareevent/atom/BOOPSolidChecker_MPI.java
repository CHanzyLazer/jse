...
/**
 * 内部使用 {@link AtomicParameterCalculator#calConnectCountBOOP_MPI} 来进行计算的固体判据器
 * @author liqa
 */
public class BOOPSolidChecker_MPI implements ISolidChecker {
    ...
    public BOOPSolidChecker_MPI setUseRatio(boolean aUseRatio) {mUseRatio = aUseRatio; return this;}
    public BOOPSolidChecker_MPI setUseRatio() {return setUseRatio(true);}
    
    @Override public ILogicalVector checkSolid(AtomicParameterCalculator aAPC) {
        try {
            ILogicalVector tIsSolid;
            if (mUseRatio) {
                IVector tConnectRatio = aAPC.calConnectRatioBOOP_MPI(mComm, mLInBOOP, mConnectThreshold, aAPC.unitLen()*mRNearestMul, mNnn);
                tIsSolid = tConnectRatio.greaterOrEqual(0.5);
                VectorCache.returnVec(tConnectRatio);
            } else {
                IVector tConnectCount = aAPC.calConnectCountBOOP_MPI(mComm, mLInBOOP, mConnectThreshold, aAPC.unitLen()*mRNearestMul, mNnn);
