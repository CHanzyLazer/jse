...
public class ForwardFluxSampling<T> extends AbstractThreadPool<ParforThreadPool> implements Runnable, IHasAutoShutdown {
    ...
    public static final byte FIXED = 0, SIMPLE_GUESS = 1, CONSERVATIVE_GUESS = 2, TIME_DEPENDENT = -1;
        ...
        @Nullable Point nextUntilReachLambdaAOrLambdaB(boolean aStatTime) {
            ...
            while (true) {
                ...
                if (mStep1Pruning && mPruningProb > 0.0) {
                    boolean tPruning;
                    if (mPruningPolicy == TIME_DEPENDENT) {
                        // 时间依赖的 pruning
    ...
    public void run() {
        ...
        if (mStep < mN) {
            ...
            if (mCutoff > 0.0) {
                ...
                while (mPointsOnLambda.size()*3>mN0*2 && UT.Code.last(mPointsOnLambda).multiple<mCutoff) {
