...
public class SimpleSphericalChebyshev extends SimpleBasis {
    ...
    private SimpleSphericalChebyshev(String @Nullable[] aSymbols, int aNumTypes, int aWType, int aNMax, int aLMax, int aLMaxMax, int aL3Max, double aRCut) {
        if (!SimpleChebyshev.VALID_WTYPE.containsValue(aWType)) throw new IllegalArgumentException("Input wtype MUST be in {0, 3}, input: "+ aWType);
        ...
        mWType = aWType;
        mNMax = aNMax;
        mSizeN = SimpleChebyshev.getSizeN_(aWType, aNumTypes, aNMax);
    ...
    SimpleSphericalChebyshev(String @Nullable[] aSymbols, int aNumTypes, int aWType, int aNMax, int aLMax, int aL3Max, double aRCut) {
        this(aSymbols, aNumTypes, aWType, aNMax, aLMax, Math.max(aLMax, aL3Max), aL3Max, aRCut);
    }
    /**
    ...
    public SimpleSphericalChebyshev(String @NotNull[] aSymbols, int aNMax, int aLMax, double aRCut) {
        this(aSymbols, aSymbols.length, WTypeBasis.WTYPE_DEFAULT, aNMax, aLMax, SphericalChebyshev.DEFAULT_L3MAX, aRCut);
    }
    /**
     ...
     * @param aRCut 截断半径
     */
    public SimpleSphericalChebyshev(int aNumTypes, int aNMax, int aLMax, double aRCut) {
        this(null, aNumTypes, WTypeBasis.WTYPE_DEFAULT, aNMax, aLMax, SphericalChebyshev.DEFAULT_L3MAX, aRCut);
    }
    
    @SuppressWarnings({"rawtypes"})
    public static SimpleSphericalChebyshev load(String @NotNull[] aSymbols, Map aMap) {
        return new SimpleSphericalChebyshev(
            aSymbols, aSymbols.length, SimpleChebyshev.getWType_(aMap),
    ...
    @SuppressWarnings({"rawtypes"})
    public static SimpleSphericalChebyshev load(int aNumTypes, Map aMap) {
        return new SimpleSphericalChebyshev(
            null, aNumTypes, SimpleChebyshev.getWType_(aMap),
            ((Number)UT.Code.getWithDefault(aMap, SphericalChebyshev.DEFAULT_NMAX, "nmax")).intValue(),
            ((Number)UT.Code.getWithDefault(aMap, SphericalChebyshev.DEFAULT_LMAX, "lmax")).intValue(),
            ((Number)UT.Code.getWithDefault(aMap, SphericalChebyshev.DEFAULT_L3MAX, "l3max")).intValue(),
            ((Number)UT.Code.getWithDefault(aMap, SphericalChebyshev.DEFAULT_RCUT, "rcut")).doubleValue()
    ...
    void calCnlm(DoubleList aNlDx, DoubleList aNlDy, DoubleList aNlDz, IntList aNlType, DoubleArrayVector rCnlm, DoubleArrayVector rForwardCache) {
        ...
        for (int j = 0; j < tNN; ++j) {
            ...
            if (mWType==WTypeBasis.WTYPE_EXFULL) {
                int ck = 0, ckwt = type*tSizeBnlm;
                for (int n = 0; n <= mNMax; ++n) {
                    double tMul = fc*rRn.get(n);
                    for (int k = 0; k < mLMAll; ++k) {
                        double tValue = tMul*rY.get(k);
                        rCnlm.add(ck, tValue);
                        rCnlm.add(ckwt, tValue);
                        ++ck; ++ckwt;
                    }
                }
            } else {
