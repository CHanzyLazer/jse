...
public class Lammpstrj extends AbstractListWrapper<SubLammpstrj, IAtomData, SubLammpstrj> {
    ...
    public List<Box> allLmpBox() {return AbstractCollections.map(mList, SubLammpstrj::lmpBox);}
     ...
     * @return 获取到的 MFPC
     */
    @ApiStatus.Experimental public MultiFrameParameterCalculator getTypeMultiFrameParameterCalculator(double aTimestep, int aType, int aThreadNum) {return MultiFrameParameterCalculator.of(AbstractCollections.map(mList, atomData -> atomData.operation().filterType(aType)), size()>1 ? aTimestep*(get(1).timeStep()-get(0).timeStep()) : aTimestep, aThreadNum);}
    @ApiStatus.Experimental public MultiFrameParameterCalculator getMultiFrameParameterCalculator    (double aTimestep                           ) {return MultiFrameParameterCalculator.of(mList                                                                             , size()>1 ? aTimestep*(get(1).timeStep()-get(0).timeStep()) : aTimestep            );}
    @ApiStatus.Experimental public MultiFrameParameterCalculator getMultiFrameParameterCalculator    (double aTimestep,            int aThreadNum) {return MultiFrameParameterCalculator.of(mList                                                                             , size()>1 ? aTimestep*(get(1).timeStep()-get(0).timeStep()) : aTimestep, aThreadNum);}
    @ApiStatus.Experimental public MultiFrameParameterCalculator getTypeMultiFrameParameterCalculator(double aTimestep, int aType                ) {return MultiFrameParameterCalculator.of(AbstractCollections.map(mList, atomData -> atomData.operation().filterType(aType)), size()>1 ? aTimestep*(get(1).timeStep()-get(0).timeStep()) : aTimestep            );}
    @ApiStatus.Experimental @VisibleForTesting public MultiFrameParameterCalculator       getMFPC    (double aTimestep                           ) {return MFPC.of(mList                                                                             , size()>1 ? aTimestep*(get(1).timeStep()-get(0).timeStep()) : aTimestep            );}
    @ApiStatus.Experimental @VisibleForTesting public MultiFrameParameterCalculator       getMFPC    (double aTimestep,            int aThreadNum) {return MFPC.of(mList                                                                             , size()>1 ? aTimestep*(get(1).timeStep()-get(0).timeStep()) : aTimestep, aThreadNum);}
    @ApiStatus.Experimental @VisibleForTesting public MultiFrameParameterCalculator       getTypeMFPC(double aTimestep, int aType                ) {return MFPC.of(AbstractCollections.map(mList, atomData -> atomData.operation().filterType(aType)), size()>1 ? aTimestep*(get(1).timeStep()-get(0).timeStep()) : aTimestep            );}
    @ApiStatus.Experimental @VisibleForTesting public MultiFrameParameterCalculator       getTypeMFPC(double aTimestep, int aType, int aThreadNum) {return MFPC.of(AbstractCollections.map(mList, atomData -> atomData.operation().filterType(aType)), size()>1 ? aTimestep*(get(1).timeStep()-get(0).timeStep()) : aTimestep, aThreadNum);}
}
