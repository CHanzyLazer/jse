package jse.optim;

import jse.math.vector.IVector;
import jse.math.vector.Vector;
import jse.math.vector.Vectors;

/**
 * <a href="https://en.wikipedia.org/wiki/Stochastic_gradient_descent#Adam">Adam 优化器</a>
 * 实现：
 * <pre> {@code
 * m_k <- b1 m_(k-1) + (1 - b1) df/dx_k
 * v_k <- b2 v_(k-1) + (1 - b2) (df/dx_k)^2
 * _m_k <- m_k / (1 - b1^k)
 * _v_k <- v_k / (1 - b2^k)
 * x_k <- x_(k-1) - eta * _m_k / (sqrt(_v_k) + eps)
 * } </pre>
 * 主要针对有一定随机性的目标函数优化
 * <p>
 * 对于 AMSGrad，采用：
 * <pre> {@code
 * _v_k <- max(_v_(k-1), v_k)
 * __v_k <- _v_k / (1 - b2^k)
 * x_k <- x_(k-1) - eta * _m_k / (sqrt(__v_k) + eps)
 * } </pre>
 *
 * @see IOptimizer
 * @author liqa
 */
public class Adam extends AbstractOptimizer {
    protected double mEta;
    protected double mBeta1, mBeta2;
    protected double mEps;
    protected boolean mAMSGrad;
    
    protected Vector mMomentum = null, mVariance = null, mVariance2 = null;
    protected double mBeta1Prod = 1.0, mBeta2Prod = 1.0;
    
    /**
     * 创建一个 Adam 优化器
     * @param aEta 使用的学习率，默认为 {@code 0.001}
     * @param aBeta1 计算梯度平均值的系数，默认为 {@code 0.9}
     * @param aBeta2 计算梯度平方平均值的系数，默认为 {@code 0.999}
     * @param aEps 在分母中添加项以提高数值稳定性，默认为 {@code 1e-8}
     * @param aAMSGrad 是否启用 AMSGrad，参考
     * <a href="https://openreview.net/forum?id=ryQu7f-RZ">On the Convergence of Adam and Beyond</a>，默认为 {@code false}
     */
    public Adam(double aEta, double aBeta1, double aBeta2, double aEps, boolean aAMSGrad) {
        mEta = aEta;
        mBeta1 = aBeta1;
        mBeta2 = aBeta2;
        mEps = aEps;
        mAMSGrad = aAMSGrad;
    }
    /**
     * 创建一个 Adam 优化器
     * @param aEta 使用的学习率，默认为 {@code 0.001}
     * @see #Adam(double, double, double, double, boolean)
     */
    public Adam(double aEta) {
        this(aEta, 0.9, 0.999, 1e-8, false);
    }
    /**
     * 创建一个 Adam 优化器
     * @see #Adam(double, double, double, double, boolean)
     */
    public Adam() {
        this(0.001);
    }
    
    /**
     * {@inheritDoc}
     * @param aLearningRate {@inheritDoc}
     * @return {@inheritDoc}
     */
    @Override public Adam setLearningRate(double aLearningRate) {
        mEta = aLearningRate;
        return this;
    }
    
    /**
     * {@inheritDoc}
     * @param aParameter {@inheritDoc}
     * @return {@inheritDoc}
     */
    @Override public Adam setParameter(IVector aParameter) {
        super.setParameter(aParameter);
        if (aParameter!=null) {
            mMomentum = Vectors.zeros(aParameter.size());
            mVariance = Vectors.zeros(aParameter.size());
            if (mAMSGrad) {
                mVariance2 = Vectors.zeros(aParameter.size());
            }
        }
        return this;
    }
    /**
     * {@inheritDoc}
     */
    @Override public void reset() {
        if (mMomentum!=null) mMomentum.fill(0.0);
        if (mVariance!=null) mVariance.fill(0.0);
        if (mVariance2!=null) mVariance2.fill(0.0);
        mBeta1Prod = 1.0;
        mBeta2Prod = 1.0;
    }
    
    /**
     * {@inheritDoc}
     * @param aStep {@inheritDoc}
     * @param aParameter {@inheritDoc}
     * @param rParameterStep {@inheritDoc}
     */
    @Override protected void calStep(int aStep, IVector aParameter, Vector rParameterStep) {
        IVector tGrad = grad();
        mMomentum.operation().operate2this(tGrad, (l, r) -> mBeta1*l + (1-mBeta1)*r);
        mVariance.operation().operate2this(tGrad, (l, r) -> mBeta2*l + (1-mBeta2)*r*r);
        mBeta1Prod *= mBeta1;
        mBeta2Prod *= mBeta2;
        if (!mAMSGrad) {
            mMomentum.operation().operate2dest(
                mVariance, rParameterStep,
                (m, v) -> -mEta * m/(1-mBeta1Prod) / (Math.sqrt(v/(1-mBeta2Prod)) + mEps)
            );
        } else {
            final int tSize = mMomentum.size();
            for (int i = 0; i < tSize; ++i) {
                double m = mMomentum.get(i);
                double v = mVariance.get(i);
                double v2 = mVariance2.get(i);
                v = Math.max(v2, v);
                rParameterStep.set(i, -mEta * m/(1-mBeta1Prod) / (Math.sqrt(v/(1-mBeta2Prod)) + mEps));
            }
            mVariance.operation().div2dest(1-mBeta2Prod, mVariance2);
            
        }
    }
}
