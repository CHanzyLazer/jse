...
/**
 * 当作 cuda 中的 {@code double *} 处理的指针，
 * 虽然一般来说 c 中的 {@code double} 和 java 的 {@code jdouble}
 * 等价，但这里为了不失一般性依旧单指 c 中的 {@code double}。
 * <p>
 * 对于大多数 GPU 来说 double 运算要远远慢于 float 运算，这里只是作为兼容提供
 * @see CudaPointer CudaPointer: 一般的 cuda 指针包装类
 * @author liqa
 */
public class DoubleCudaPointer extends CudaPointer implements IDoubleOrFloatCudaPointer {
    /**
     * 直接从一个任意的 cuda 指针初始化一个 {@link DoubleCudaPointer} 对象
     * @param aPtr 需要包装的 cuda 指针值
     */
    @ApiStatus.Internal public DoubleCudaPointer(long aPtr) {super(aPtr);}
    
    /**
     * 调用 {@link CudaCore#cudaMalloc} 来分配内存创建一个 cuda 指针
     *
     * @param aCount 需要分配的数组长度，即等价于 {@code malloc(aCount*sizeof(float))}
     * @return 创建的双精度浮点 cuda 指针对象
     */
    @UnsafeJNI("Manual free required")
    public static DoubleCudaPointer malloc(long aCount) throws CudaException {
        return new DoubleCudaPointer(CudaCore.cudaMalloc(aCount*TYPE_SIZE));
    }
    ...
    public final static long TYPE_SIZE = DoubleCPointer.TYPE_SIZE;
    ...
    public void fill(IDataShell<double[]> aData) throws CudaException {
    ...
    public void fill(double[] aData, int aStart, int aCount) throws CudaException {
        if (aCount==0) return;
        if (isNull()) throw new NullPointerException();
        CPointer.rangeCheck(aData.length, aStart+aCount);
        fill0(mPtr, aData, aStart, aCount);
    }
    private native static void fill0(long rPtr, double[] aData, int aStart, int aCount) throws CudaException;
    ...
    private native static void fillF0(long rPtr, float[] aData, int aStart, int aCount) throws CudaException;
     ...
     */
    @UnsafeJNI("Invalid input count may directly result in JVM SIGSEGV")
    public void fill(DoubleCPointer aData, long aCount) throws CudaException {
        if (aCount==0) return;
    ...
    @Override public void fillF(FloatCPointer aData, long aCount) throws CudaException {
        ...
        fillF1(mPtr, aData.ptr_(), aCount);
    }
    private native static void fillF1(long rPtr, long aData, long aCount) throws CudaException;
    /**
     ...
     */
    @UnsafeJNI("Invalid input count may directly result in JVM SIGSEGV")
    public void fill(DoubleCudaPointer aData, long aCount) throws CudaException {
        if (aCount==0) return;
        memcpy2this(aData, aCount*TYPE_SIZE);
    }
     ...
     */
    @UnsafeJNI("Invalid input count may directly result in JVM SIGSEGV")
    public void parse2dest(DoubleCPointer rDest, long aCount) throws CudaException {
        if (aCount==0) return;
    ...
    @Override public void parse2destF(FloatCPointer rDest, long aCount) throws CudaException {
        ...
        parse2destF1(mPtr, rDest.ptr_(), aCount);
    }
    private native static void parse2destF1(long aPtr, long rDest, long aCount) throws CudaException;
    /**
     ...
     */
    @UnsafeJNI("Invalid input count may directly result in JVM SIGSEGV")
    public void parse2dest(DoubleCudaPointer rDest, long aCount) throws CudaException {
        if (aCount==0) return;
        memcpy2dest(rDest, aCount*TYPE_SIZE);
    }
     ...
     * @return {@inheritDoc}
     */
    @Override public DoubleCudaPointer plus(long aCount) {
        if (isNull()) throw new NullPointerException();
        return new DoubleCudaPointer(rightShift0(mPtr, aCount));
    }
     ...
     * @return {@inheritDoc}
     */
    @Override public DoubleCudaPointer minus(long aCount) {
        if (isNull()) throw new NullPointerException();
        return new DoubleCudaPointer(leftShift0(mPtr, aCount));
    }
     ...
     * @return {@inheritDoc}
     */
    @Override public DoubleCudaPointer copy() {
        return new DoubleCudaPointer(mPtr);
    }
}
