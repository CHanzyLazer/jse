...
import jse.system.SSHSystemExecutor;
...
public final class ServerSSH implements IAutoShutdown {
    ...
    public void save(String aFilePath, String aKey) throws Exception {
        Map rJson = new LinkedHashMap();
        save(rJson);
        jse.io.Encryptor tEncryptor = new jse.io.Encryptor(aKey);
    ...
    public static ServerSSH load(String aFilePath, String aKey) throws Exception {
        jse.io.Decryptor tDecryptor = new jse.io.Decryptor(aKey);
    ...
    /// 实用方法
    // 提交命令
    public jse.code.task.Task task_system(final String aCommand) {return new jse.code.task.SerializableTask(() -> {system(aCommand); return true;}) {
    ...
    // 上传目录到服务器
    public jse.code.task.Task task_putDir(final String aDir) {return new jse.code.task.SerializableTask(() -> {putDir(aDir); return true;}) {
    ...
    // 从服务器下载目录
    public jse.code.task.Task task_getDir(final String aDir) {return new jse.code.task.SerializableTask(() -> {getDir(aDir); return true;}) {
    ...
    public jse.code.task.Task task_clearDir(final String aDir) {return new jse.code.task.SerializableTask(() -> {clearDir(aDir); return true;}) {
    ...
    // 递归删除远程服务器的文件夹
    public jse.code.task.Task task_rmdir(final String aDir) {return task_removeDir(aDir);}
    public jse.code.task.Task task_removeDir(final String aDir) {return new jse.code.task.SerializableTask(() -> {rmdir(aDir); return true;}) {
    ...
    public jse.code.task.Task task_mkdir(final String aDir) {return task_makeDir(aDir);}
    public jse.code.task.Task task_makeDir(final String aDir) {return new jse.code.task.SerializableTask(() -> {mkdir(aDir); return true;}) {
    ...
    // 上传单个文件
    public jse.code.task.Task task_putFile(final String aFilePath) {return new jse.code.task.SerializableTask(() -> {putFile(aFilePath); return true;}) {
    ...
    // 下载单个文件
    public jse.code.task.Task task_getFile(final String aFilePath) {return new jse.code.task.SerializableTask(() -> {getFile(aFilePath); return true;}) {
    ...
    // 上传目录到服务器的并发版本，理论会更快
    public jse.code.task.Task task_putDir(final String aDir, final int aThreadNumber) {return new jse.code.task.SerializableTask(() -> {putDir(aDir, aThreadNumber); return true;}) {
    ...
    // 从服务器下载目录的并发版本，理论会更快
    public jse.code.task.Task task_getDir(final String aDir, final int aThreadNumber) {return new jse.code.task.SerializableTask(() -> {getDir(aDir, aThreadNumber); return true;}) {
    ...
    public jse.code.task.Task task_clearDir(final String aDir, final int aThreadNumber) {return new jse.code.task.SerializableTask(() -> {clearDir(aDir, aThreadNumber); return true;}) {
    ...
    public jse.code.task.Task task_putWorkingDir() {return new jse.code.task.SerializableTask(() -> {putWorkingDir(); return true;}) {
        @Override public String toString() {return Type.PUT_WORKING_DIR.name();}
    };}
    public jse.code.task.Task task_putWorkingDir(final int aThreadNumber) {return new jse.code.task.SerializableTask(() -> {putWorkingDir(aThreadNumber); return true;}) {
    ...
    public jse.code.task.Task task_getWorkingDir() {return new jse.code.task.SerializableTask(() -> {getWorkingDir(); return true;}) {
        @Override public String toString() {return Type.GET_WORKING_DIR.name();}
    };}
    public jse.code.task.Task task_getWorkingDir(final int aThreadNumber) {return new jse.code.task.SerializableTask(() -> {getWorkingDir(aThreadNumber); return true;}) {
    ...
    public jse.code.task.Task task_clearWorkingDir() {return new jse.code.task.SerializableTask(() -> {clearWorkingDir(); return true;}) {
        @Override public String toString() {return Type.CLEAR_WORKING_DIR.name();}
    };}
    public jse.code.task.Task task_clearWorkingDir(final int aThreadNumber) {return new jse.code.task.SerializableTask(() -> {clearWorkingDir(aThreadNumber); return true;}) {
