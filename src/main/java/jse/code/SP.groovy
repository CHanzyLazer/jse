...
class SP {
     ...
     * @author liqa
     */
    static class JupyterKernel extends BaseKernel {
        ...
        private static final CharPredicate ID_CHAR = CharPredicate.builder()
            .inRange((char)'a', (char)'z')
            .inRange((char)'A', (char)'Z')
            .inRange((char)'0', (char)'9')
            .match((char)'_')
            .build()
        ...
        private static String toDisplayString(@Nullable Object aObj) {
            ...
            case GString    -> {'"'+aObj+'"'}
            case Iterable   -> {
                def rBuilder = new StringBuilder()
                rBuilder << '['
                def it = aObj.iterator()
                if (it.hasNext()) {
                    rBuilder << toDisplayString(it.next())
                }
                while (it.hasNext()) {
                    rBuilder << ', '
                    rBuilder << toDisplayString(it.next())
            ...
            case Map        -> {toDisplayString((aObj as Map).entrySet())}
            case Map.Entry  -> {
                def tEntry = aObj as Map.Entry
                toDisplayString(tEntry.key)+':'+toDisplayString(tEntry.value)
            }
            case Object[]   -> {toDisplayString(AbstractCollections.from(aObj as Object[]))}
            default         -> {String.valueOf(aObj)}
            }
        }
        ...
        JupyterKernel() {
            ...
            mLanguageInfo = new LanguageInfo.Builder('groovy')
                ...
                .build()
            mBanner = """\
jse $VERSION (groovy: $GroovySystem.version, java: ${System.properties['java.version']})
Protocol v$Header.PROTOCOL_VERISON implementation by ${KERNEL_META['project']?:'UNKNOWN'} ${KERNEL_META['version']?:'UNKNOWN'}\
"""
            mHelpLinks = [
        ...
        @Override LanguageInfo getLanguageInfo() {return mLanguageInfo}
        @Override String getBanner() {return mBanner}
        @Override List<LanguageInfo.Help> getHelpLinks() {return mHelpLinks}
...
'''
        ...
        @Override DisplayData eval(String expr) throws Exception {
            // 先处理 cell magics
            @Nullable CellMagicParseContext tCellCTX = mMagicParser.parseCellMagic(expr)
            boolean tIsPython = false
            if (tCellCTX != null) {
                String tCellName = tCellCTX.magicCall.name
                switch (tCellName) {
                case 'python': {tIsPython = true; break}
                case 'groovy': {break}
                case 'writefile': {
                    def tArgs = tCellCTX.magicCall.args
                    if (tArgs.empty) throw new IllegalArgumentException('Magic `%%writefile` Must have at least one argument.')
                    String tPath = tArgs.first()
                    UT.IO.write(tPath, tCellCTX.magicCall.body)
                ...
                expr = tCellCTX.magicCall.body
            }
            // 再处理 line magics
            expr = mMagicParser.transformLineMagics(expr) {ctx ->
                String tLineName = ctx.magicCall.name
                if (tLineName == 'lsmagic') {
...
''')
                    for (Object tImage : Python.getValue('__jse_images__', List.class)) {
                        def tDisplayData = new DisplayData()
            ...
            // 再绘制 groovy 的图像
            if (!KERNEL_SHOW_FIGURE && !IPlotter.SHOWED_PLOTTERS.empty) {
                for (IPlotter tPlt : IPlotter.SHOWED_PLOTTERS) {
                    def tDisplayData = new DisplayData()
                    tDisplayData.putData(MIMEType.IMAGE_PNG, Base64.mimeEncoder.encodeToString(tPlt.encode()))
                    display(tDisplayData)
                }
                // 绘制完成清空存储
        ...
        @Override List<String> formatError(Exception e) {
        ...
        @Override DisplayData inspect(String code, int at, boolean extraDetail) throws Exception {
            ...
            boolean tIsPython = tCellCTX!=null && tCellCTX.magicCall.name=='python'
        ...
        @Override ReplacementOptions complete(String code, int at) throws Exception {
            StringSearch.Range tMatch = StringSearch.findLongestMatchingAt(code, at, ID_CHAR)
            if (tMatch == null) return null
            @Nullable CellMagicParseContext tCellCTX = mMagicParser.parseCellMagic(code)
            boolean tIsPython = tCellCTX!=null && tCellCTX.magicCall.name=='python'
            String tPrefix = tMatch.extractSubString(code)
            return new ReplacementOptions((tIsPython ? PYTHON_AUTO_COMPLETER : GROOVY_AUTO_COMPLETER).autocomplete(tPrefix), tMatch.low, tMatch.high)
        }
    }
    ...
    static void run(String aScriptPath, String... aArgs) throws Exception {runScript(aScriptPath, aArgs)}
    ...
    static class Groovy {
        ...
        @CompileStatic
        final static class GroovyObjectWrapper implements GroovyObject {
            private final GroovyObject mObj
            
            @PackageScope GroovyObjectWrapper(GroovyObject aObj) {mObj = aObj}
            
            @Override Object invokeMethod(String name, Object args) {return of(mObj.invokeMethod(name, args))}
            @Override Object getProperty(String propertyName) {return of(mObj.getProperty(propertyName))}
            @Override void setProperty(String propertyName, Object newValue) {mObj.setProperty(propertyName, newValue)}
            @Override MetaClass getMetaClass() {return mObj.getMetaClass()}
            @Override void setMetaClass(MetaClass metaClass) {mObj.setMetaClass(metaClass)}
            
            @Override String toString() {return mObj.toString()}
            
            GroovyObject unwrap() {return mObj}
            
            /** 主要用来判断是否需要外包这一层 */
            static Object of(Object aObj) {return (!(aObj instanceof GroovyObjectWrapper) && (aObj instanceof GroovyObject)) ? (new GroovyObjectWrapper((GroovyObject)aObj)) : aObj}
        }
        ...
        static void runShell() throws Exception {
            ...
            // 这样手动指定 CLASS_LOADER
            Groovysh tGroovysh = new Groovysh(GROOVY_SHELL.classLoader, GROOVY_SHELL.context, new IO(), null, GROOVY_CONF)
            // 这样直接添加默认 import，shell 会默认导入这些方便使用
            tGroovysh.imports 
                << AbstractAtoms.class.name
                << Structures.class.name
                << AbstractCollections.class.name
                << Iterables.class.name
                << NewCollections.class.name
                << ArrayLists.class.name
                << InFiles.class.name
                << IOFiles.class.name
                << MathEX.class.name
                << ComplexDouble.class.name
                << Func1.class.name
                << Matrices.class.name
                << Tables.class.name
                << Vectors.class.name
                << Plotters.class.name
                << ("static $UT.Timer.class.name.*" as String)
                << ("static $UT.Par.class.name.*"   as String)
                << ("static $UT.Exec.class.name.*"  as String)
                << ("static $UT.IO.class.name.*"    as String)
                << ("static $UT.Math.class.name.*"  as String)
                << ("static $UT.Plot.class.name.*"  as String)
                << ("static $UT.Code.class.name.*"  as String)
                << ("static $CS.class.name.*"       as String)
                << ("static $CS.Exec.class.name.*"  as String)
            tGroovysh.run(null)
        }
        
        /** python like stuffs，exec 不会获取返回值，eval 获取返回值 */
        static void exec(String aText) throws Exception {GROOVY_SHELL.evaluate(aText, "ScriptJSE${COUNTER.incrementAndGet()}.groovy")}
        static void execFile(String aFilePath) throws Exception {GROOVY_SHELL.evaluate(toSourceFile(aFilePath))}
        static Object eval(String aText) throws Exception {return GroovyObjectWrapper.of(GROOVY_SHELL.evaluate(aText, "ScriptJSE${COUNTER.incrementAndGet()}.groovy"))}
        static Object evalFile(String aFilePath) throws Exception {return GroovyObjectWrapper.of(GROOVY_SHELL.evaluate(toSourceFile(aFilePath)))}
        
        /** 直接运行文本的脚本 */
        static Object runText(String aText, String... aArgs) throws Exception {return GroovyObjectWrapper.of(GROOVY_SHELL.run(aText, "ScriptJSE${COUNTER.incrementAndGet()}.groovy", aArgs))}
        /** Groovy 现在也可以使用 getValue 来获取变量以及 setValue 设置变量（仅限于 Context 变量，这样可以保证效率） */
        static Object get(String aValueName) throws Exception {return getValue(aValueName)}
        static void set(String aValueName, Object aValue) throws Exception {setValue(aValueName, aValue)}
        static void remove(String aValueName) throws Exception {removeValue(aValueName)}
        static boolean hasValue(String aValueName) throws Exception {return GROOVY_SHELL.context.hasVariable(aValueName)}
        static Object getValue(String aValueName) throws Exception {return GroovyObjectWrapper.of(GROOVY_SHELL.context.getVariable(aValueName))}
        static void setValue(String aValueName, Object aValue) throws Exception {GROOVY_SHELL.context.setVariable(aValueName, aValue)}
        static void removeValue(String aValueName) throws Exception {GROOVY_SHELL.context.removeVariable(aValueName)}
        /** 运行脚本文件 */
        static Object run(String aScriptPath, String... aArgs) throws Exception {return runScript(aScriptPath, aArgs)}
        static Object runScript(String aScriptPath, String... aArgs) throws Exception {return GroovyObjectWrapper.of(GROOVY_SHELL.run(toSourceFile(aScriptPath), aArgs))}
        /** 调用指定脚本中的方法 */
        static Object invoke(String aScriptPath, String aMethodName, Object... aArgs) throws Exception {
            // 获取脚本的类
            Class<?> tScriptClass = GROOVY_SHELL.classLoader.parseClass(toSourceFile(aScriptPath))
        ...
        /** 创建脚本类的实例 */
        static Object newInstance(String aScriptPath, Object... aArgs) throws Exception {
            // 获取脚本的类
            Class<?> tScriptClass = GROOVY_SHELL.classLoader.parseClass(toSourceFile(aScriptPath))
            // 获取 ScriptClass 的实例
            final Object[] fArgs = (aArgs == null) ? new Object[0] : aArgs
            ...
            if (tConstructor == null) throw new GroovyRuntimeException('Cannot find constructor with compatible args: ' + tScriptClass.name)
        ...
        static void close() throws IOException {if (GROOVY_SHELL != null) {GROOVY_SHELL.classLoader.close(); GROOVY_SHELL = null}}
        static boolean isClosed() {return GROOVY_SHELL == null}
        /** 提供一个手动刷新 GROOVY_INTERP 的接口，可以将关闭的重新打开，清除缓存和文件的依赖 */
        static void refresh() throws IOException {close(); initInterpreter_()}
        ...
        private static void initInterpreter_() {
            ...
            GROOVY_SHELL = new GroovyShell(SP.class.classLoader, new Binding(), GROOVY_CONF)
            // 指定默认的 Groovy 脚本的类路径
            GROOVY_SHELL.classLoader.addClasspath(UT.IO.toAbsolutePath(GROOVY_SP_DIR))
        }
    }
    ...
    static class Python {
        ...
        final static class InitHelper {
            ...
            static boolean initialized() {return INITIALIZED}
        ...
        /** python like stuffs，exec 不会获取返回值，eval 获取返回值 */
        static void exec(String aText) throws JepException {JEP_INTERP.exec(aText)}
        static void execFile(String aFilePath) throws JepException, IOException {JEP_INTERP.runScript(validScriptPath(aFilePath))}
        /** 由于 jep 的特性，这里可以直接使用 getValue 指定 eval */
        static Object eval(String aText) throws JepException {return JEP_INTERP.getValue(aText)}
        // python 脚本文件不会有返回值
        
        /** 直接运行文本的脚本 */
        static void runText(String aText, String... aArgs) throws JepException {setArgs_('', aArgs); JEP_INTERP.exec(aText)}
        /** Python 还可以使用 getValue 来获取变量以及 setValue 设置变量（原则上同样仅限于 Context 变量，允许可以超出 Context 但是不保证支持） */
        static Object get(String aValueName) throws JepException {return getValue(aValueName)}
        static void set(String aValueName, Object aValue) throws JepException {setValue(aValueName, aValue)}
        static void remove(String aValueName) throws JepException {removeValue(aValueName)}
        static boolean hasValue(String aValueName) throws JepException {return (Boolean)JEP_INTERP.getValue("('$aValueName' in globals()) or ('$aValueName' in locals())")}
        static Object getValue(String aValueName) throws JepException {return JEP_INTERP.getValue(aValueName)}
        static <T> T getValue(String aValueName, Class<T> aClazz) throws JepException {return JEP_INTERP.getValue(aValueName, aClazz)}
        static void setValue(String aValueName, Object aValue) throws JepException {JEP_INTERP.set(aValueName, aValue)}
        static void removeValue(String aValueName) throws JepException {JEP_INTERP.exec("del $aValueName")}
        /** 运行脚本文件 */
        static void run(String aScriptPath, String... aArgs) throws JepException, IOException {runScript(aScriptPath, aArgs)}
        ...
        static Object invoke(String aMethodName, Object... aArgs) throws JepException {
            ...
            if (aArgs.length > 1 && (aArgs.last() instanceof Map)) {
                ...
                return JEP_INTERP.invoke(aMethodName, tArgs, (Map<String, Object>)aArgs.last())
            }
            return JEP_INTERP.invoke(aMethodName, aArgs)
        }
        /** 创建 Python 实例，这里可以直接将类名当作函数调用即可 */
        static Object newInstance(String aClassName, Object... aArgs) throws JepException {return invoke(aClassName, aArgs)}
        
        /** 提供一个手动关闭 JEP_INTERP 的接口 */
        static void close() throws JepException {if (JEP_INTERP != null) {JEP_INTERP.close(); JEP_INTERP = null}}
        static boolean isClosed() {return JEP_INTERP == null}
        ...
        static {
            ...
            jep.SharedInterpreter.setConfig(new JepConfig()
                ...
                .setClassLoader(SP.class.classLoader)
...
''')
        ...
        static void downloadPackage(String aRequirement, boolean aIncludeDep, String aPlatform, String aPythonVersion) {
            // 组装指令
            List<CharSequence> rCommand = []
            rCommand << 'pip' << 'download'
            // 是否顺便下载依赖的库，这里默认不会下载依赖库，因为问题会很多
            if (!aIncludeDep) rCommand << '--no-deps'
            ...
            if (aPlatform != null && !aPlatform.isEmpty()) {
                if (aIncludeDep) {
                    rCommand << '--only-binary' << ':all:'
                    tOnlyBinary = true
                }
                rCommand << '--platform' << aPlatform
            }
            if (aPythonVersion!=null && !aPythonVersion.isEmpty()) {
                if (aIncludeDep && !tOnlyBinary) {
                    rCommand << '--only-binary' << ':all:'
                }
                rCommand << '--python-version' << aPythonVersion
            }
            // 不提供强制仅下载源码的选项，因为很多下载的源码都不能编译成功
            // 设置目标路径
            rCommand << '--dest' << "'$PYPKG_DIR'"
            // 设置需要的包名
            rCommand << "'$aRequirement'"
            
            // 直接通过系统指令执行 pip 来下载
            EXE.system(String.join(' ', rCommand))
        }
        static void downloadPackage(String aRequirement, String aPlatform, String aPythonVersion) {downloadPackage(aRequirement, false, aPlatform, aPythonVersion)}
        static void downloadPackage(String aRequirement, String aPlatform) {downloadPackage(aRequirement, aPlatform, null)}
        static void downloadPackage(String aRequirement, boolean aIncludeDep, String aPlatform) {downloadPackage(aRequirement, aIncludeDep, aPlatform, null)}
        static void downloadPackage(String aRequirement, boolean aIncludeDep) {downloadPackage(aRequirement, aIncludeDep, null)}
        static void downloadPackage(String aRequirement) {downloadPackage(aRequirement, false)}
        
        /** 基于 pip 的 python 包管理，直接安装指定包到 lib */
        static void installPackage(String aRequirement, boolean aIncludeDep, boolean aIncludeIndex) {
            // 组装指令
            List<CharSequence> rCommand = []
            rCommand << 'pip' << 'install'
            // 是否顺便下载依赖的库，这里默认不会下载依赖库，因为问题会很多
            if (!aIncludeDep) rCommand << '--no-deps'
            // 是否开启联网，这里默认不开启联网，因为标准下会使用 downloadPackage 来下载包
            if (!aIncludeIndex) rCommand << '--no-index'
            // 添加 .pypkg 到搜索路径
            rCommand << '--find-links' << "'file:$PYPKG_DIR'"
            // 设置目标路径
            rCommand << '--target' << "'$PYLIB_DIR'"
            // 强制开启更新，替换已有的包
            rCommand << '--upgrade'
            // 设置需要的包名
            rCommand << "'$aRequirement'"
        ...
        static void installPackage(String aRequirement, boolean aIncludeDep) {installPackage(aRequirement, aIncludeDep, false)}
        static void installPackage(String aRequirement) {installPackage(aRequirement, false)}
        
        
        private static String cmakeInitCmdJep_(String aJepBuildDir) {
            // 设置参数，这里使用 List 来构造这个长指令
            List<CharSequence> rCommand = []
            rCommand << 'cd' << "'$aJepBuildDir'" << ';'
            rCommand << 'cmake'
            // 这里设置 C 编译器（如果有）
            if (Conf.CMAKE_C_COMPILER != null) {rCommand << '-D' << "CMAKE_C_COMPILER=$Conf.CMAKE_C_COMPILER"}
            if (Conf.CMAKE_C_FLAGS    != null) {rCommand << '-D' << "CMAKE_C_FLAGS='$Conf.CMAKE_C_FLAGS'"}
        ...
        private static String cmakeSettingCmdJep_(String aJepBuildDir) throws IOException {
            ...
            rCommand << '-D' << "CMAKE_ARCHIVE_OUTPUT_DIRECTORY:PATH='$JEP_LIB_DIR'"
            rCommand << '-D' << "CMAKE_LIBRARY_OUTPUT_DIRECTORY:PATH='$JEP_LIB_DIR'"
            rCommand << '-D' << "CMAKE_RUNTIME_OUTPUT_DIRECTORY:PATH='$JEP_LIB_DIR'"
            rCommand << '-D' << "CMAKE_ARCHIVE_OUTPUT_DIRECTORY_RELEASE:PATH='$JEP_LIB_DIR'"
            rCommand << '-D' << "CMAKE_LIBRARY_OUTPUT_DIRECTORY_RELEASE:PATH='$JEP_LIB_DIR'"
            rCommand << '-D' << "CMAKE_RUNTIME_OUTPUT_DIRECTORY_RELEASE:PATH='$JEP_LIB_DIR'"
            rCommand << '.'
            return String.join(' ', rCommand)
        }
        ...
        private static @NotNull String installJep_() throws Exception {
            ...
            // 最后进行构造操作
            EXE.system("cd '$tJepBuildDir'; cmake --build . --config Release")
        ...
        static void installAse() throws IOException {
            ...
            if (!tHasAsePkg) {
                System.out.println("ASE INIT INFO: No ase package in $PYPKG_DIR, downloading...")
