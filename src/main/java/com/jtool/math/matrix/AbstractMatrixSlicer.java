...
public abstract class AbstractMatrixSlicer implements IMatrixSlicer {
    ...
    @Override public final IMatrix get(IIndexFilter  aSelectedRows, int[]         aSelectedCols) {return get(aSelectedRows.fixedFilter(thisRowNum_()), aSelectedCols);}
    @Override public final IMatrix get(IIndexFilter  aSelectedRows, List<Integer> aSelectedCols) {return get(aSelectedRows.fixedFilter(thisRowNum_()), aSelectedCols);}
    @Override public final IMatrix get(IIndexFilter  aSelectedRows, SliceType     aSelectedCols) {return get(aSelectedRows.fixedFilter(thisRowNum_()), aSelectedCols);}
    @Override public final IVector get(IIndexFilter  aSelectedRows, int           aSelectedCol ) {return get(aSelectedRows.fixedFilter(thisRowNum_()), aSelectedCol);}
    @Override public final IMatrix get(int[]         aSelectedRows, IIndexFilter  aSelectedCols) {return get(aSelectedRows, aSelectedCols.fixedFilter(thisColNum_()));}
    @Override public final IMatrix get(List<Integer> aSelectedRows, IIndexFilter  aSelectedCols) {return get(aSelectedRows, aSelectedCols.fixedFilter(thisColNum_()));}
    @Override public final IMatrix get(SliceType     aSelectedRows, IIndexFilter  aSelectedCols) {return get(aSelectedRows, aSelectedCols.fixedFilter(thisColNum_()));}
    @Override public final IVector get(int           aSelectedRow , IIndexFilter  aSelectedCols) {return get(aSelectedRow , aSelectedCols.fixedFilter(thisColNum_()));}
    @Override public final IMatrix get(IIndexFilter  aSelectedRows, IIndexFilter  aSelectedCols) {return get(aSelectedRows.fixedFilter(thisRowNum_()), aSelectedCols.fixedFilter(thisColNum_()));}
