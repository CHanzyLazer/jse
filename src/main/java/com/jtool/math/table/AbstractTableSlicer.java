...
public abstract class AbstractTableSlicer implements ITableSlicer {
    ...
    @Override public final ITable get(IIndexFilter  aSelectedRows, int[]           aSelectedCols) {return get(aSelectedRows.fixedFilter(thisRowNum_()), aSelectedCols);}
    @Override public final ITable get(IIndexFilter  aSelectedRows, List<Integer>   aSelectedCols) {return get(aSelectedRows.fixedFilter(thisRowNum_()), aSelectedCols);}
    @Override public final ITable get(IIndexFilter  aSelectedRows, SliceType       aSelectedCols) {return get(aSelectedRows.fixedFilter(thisRowNum_()), aSelectedCols);}
    @Override public final ITable get(IIndexFilter  aSelectedRows, int             aSelectedCol ) {return get(aSelectedRows.fixedFilter(thisRowNum_()), aSelectedCol);}
    @Override public final ITable get(int[]         aSelectedRows, IFilter<String> aSelectedCols) {return get(aSelectedRows, S2L(aSelectedCols.filter(thisHeads_())));}
    @Override public final ITable get(List<Integer> aSelectedRows, IFilter<String> aSelectedCols) {return get(aSelectedRows, S2L(aSelectedCols.filter(thisHeads_())));}
    @Override public final ITable get(SliceType     aSelectedRows, IFilter<String> aSelectedCols) {return get(aSelectedRows, S2L(aSelectedCols.filter(thisHeads_())));}
    @Override public final ITable get(int           aSelectedRow , IFilter<String> aSelectedCols) {return get(aSelectedRow , S2L(aSelectedCols.filter(thisHeads_())));}
    @Override public final ITable get(IIndexFilter  aSelectedRows, IFilter<String> aSelectedCols) {return get(aSelectedRows.fixedFilter(thisRowNum_()), S2L(aSelectedCols.filter(thisHeads_())));}
